...
function RegisterAdminMenuBar(){
	...
	if(Auth::User()->role->id == 1){
		...
		if (Auth()->user()->can('payment.settings')) {
			if (Amcoders\Plugin\Plugin::is_active('paymentgetways')) {
	...
	if(Auth::User()->role->id == 3){
		...
		if (Amcoders\Plugin\Plugin::is_active('shop')) {
            ...
            $data['Delivery Locations']=array(
                'name' => __('Delivery Locations'),
                'icon' => 'fas fa-map-marked-alt',
                'active' => Request::is('store/delivery-location*'),
                'child'=> array(
                    __('Add New') => route('store.delivery-location.create'),
                    __('Manage Locations') => route('store.delivery-location.index')
                )
            );
