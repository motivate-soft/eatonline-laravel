...
class CheckoutController extends Controller
{
    public function index(Request $request)
    {
        if(Session::has('restaurant_id') && Session::has('restaurant_cart'))
        {
            ...
            $deliveryLocations = Deliverylocation::where("restaurantId", $resturent_info->id)->get();

            return view('theme::checkout.index',compact('resturent_info','json','currency','ordertype','km_rate','credentials', 'deliveryLocations'));
        }else{
            return back();
        }
    }
...
//            $data['image'] = $image_path;
    ...
    public function confirmation()
    {
        ...
        if(Session::has('confirmOrderId'))
        {
            $confirmOrderId = Session::get('confirmOrderId');
        }
        return view('theme::checkout.confirm', compact('confirmOrderId'));
    }
}
