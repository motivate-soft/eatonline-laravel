				...
				<div class="card">
					<div class="card-body">
						<div class="table-responsive">
							...
							<p><b>Delivery Fee: </b>  {{ strtoupper($currency->value) }} {{ $info->delivery_fee }}</p>
						...
						<div class="profile-widget">
							...
							<div class="profile-widget-header">                     
								...
								<div class="profile-widget-description">
									...
									<div class="profile-widget-name">{{ __('Fulfillment Time') }}: <div class="text-muted d-inline font-weight-normal">
										@if($info->status != 3)
											{{__('Not accepted')}}
										@else
											@if($info->fulfillment_time == 0)
												{{__('Not set')}}
											@else
												<span id="fulfillment-time"></span>
												@php
													$dateTime = new DateTime($info->updated_at);
													$dateTime->modify("+". $info->fulfillment_time ." minutes");
													$fulfillment_time = $dateTime->format("Y-m-d H:i:s");
												@endphp
											@endif
										@endif
									</div></div>
...
@php
	$customerInfo=json_decode($info->data);
@endphp
<script async defer src="https://maps.googleapis.com/maps/api/js?key={{ config('custom.PLACE_KEY') }}&libraries=places&sensor=false&callback=initialise"></script>
<script>
	"use strict";
...
@section('custom_js')
	...
	<script>
        ...
        var fulfillment_time_amount = '{{$info->fulfillment_time}}';
        var order_status = '{{$info->status}}';
        var server_ftime = '{{$fulfillment_time}}';
        var client_fMilliseconds = 0;
        var server_fmilliseconds = 0;
        var now_time = new Date();
        var client_timezone = now_time.getTimezoneOffset();
        var client_fulfillment_time = "";

        get_fulfillment_time();

        function get_fulfillment_time() {

            if(fulfillment_time_amount == 0) {
			    return false;
			}
            ...
            server_fmilliseconds = new Date(server_ftime).getTime();

            client_fMilliseconds = parseInt(server_fmilliseconds) - client_timezone * 60 * 1000;

            client_fulfillment_time = new Date(client_fMilliseconds);

			var t_hour = client_fulfillment_time.getHours();
			var t_mins = client_fulfillment_time.getMinutes();
			...
			var formated_time = t_hour + ":" + t_mins;

            $("#fulfillment-time").html(formated_time);
		}
	</script>
@endsection()