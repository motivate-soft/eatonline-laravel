				...
				<table class="table">
					<thead>
						<tr>
							...
							<th class="am-tags">{{ __('Fulfillment Time') }}</th>
						...
						<tr>
							...
							<td>
								@if($order->status != 3)
									{{__('Not accepted')}}
								@else
									@if($order->fulfillment_time == 0)
										{{__('Not set')}}
									@else
										{{ $order->fulfillment_time . "minutes"}}
										{{--@php--}}
											{{--$dateTime = new DateTime($order->updated_at);--}}
                                            {{--$dateTime->modify("+". $order->fulfillment_time ." minutes");--}}
                                            {{--echo $dateTime->format("Y-m-d H:i") @endphp--}}
									@endif
								@endif
							</td>
